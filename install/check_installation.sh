...
do 
    ...
    conda activate ${ENV_NAME} && if [ -z "${VEBA_DATABASE}" ]; then echo "[Fail] VEBA_DATABASE environment variable NOT set in ${ENV_NAME}"; else echo "[Pass] VEBA_DATABASE environment SUCCESSFULLY set in ${ENV_NAME}"; fi
done
