...
def main(args=None):
    ...
    df_completeness = pd.read_csv(opts.completeness_tsv, sep="\t", index_col=0).drop(["contig_length", "proviral_length", "kmer_freq"], axis=1) # Duplicates
    ...
    if df_checkv.empty:
        ...
        # df_checkv.columns =  ["id_contig"] + df_checkv.columns.tolist()
        df_checkv.index.name = "id_virus"
        df_checkv.to_csv(os.path.join(opts.output_directory, "quality_summary.filtered.tsv" ), sep="\t") # This is empty
        ...
        # Rename to virus name
        df_checkv.index = df_checkv.index.map(lambda x: contig_to_virus[x])
