...
def main(args=None):
    ...
    for fp in pv(glob.glob(os.path.join(directories["intermediate"], "*", "genome_clusters.tsv")), "Running MMSEQS2"):
        ...
        for id_genomecluster, data in mag_to_genomecluster_within_organism_type.groupby(mag_to_genomecluster_within_organism_type):
            ...
            protein_to_proteinluster_within_organism_type = pd.read_csv(os.path.join(genomecluster_directory, "protein_clusters.tsv"), sep="\t", index_col=0, header=None).iloc[:,0]
